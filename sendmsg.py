...
class qianniu:
    def __init__(self):
        '''实例热键、线程类，并通过线程启动热键'''
        self.a021=weiabc123.weiSearch_hotkey()
        self.xian4cheng21=weiabc123.weiThreading()
        self.xian4cheng21.wei_start(self.a021.wei_start,1,118)

    def find_jb(self):#7.0版本
        '''获取需要的句柄'''
        self.wwsskjb=self.juBingChuangKou.wei_childrenHandle(self.juBingChuangKou.wei_childrenHandle(self.hwnd)[5])[0]#旺旺搜索框
        self.wwsskscjb=self.juBingChuangKou.wei_childrenHandle(self.juBingChuangKou.wei_childrenHandle(self.hwnd)[5])[2]#旺搜索框关闭（右）
        #self.djss=self.juBingChuangKou.wei_childrenHandle(self.hwnd)[2]#点击搜索
        guodu=win32gui.FindWindowEx(self.juBingChuangKou.wei_childrenHandle(self.hwnd)[3], None, 'SplitterBar',None)
        self.xxsrkjb=win32gui.FindWindowEx(self.juBingChuangKou.wei_childrenHandle(guodu)[1], None, 'RichEditComponent',None)
        self.fsjb=win32gui.FindWindowEx(self.juBingChuangKou.wei_childrenHandle(guodu)[1], None, 'StandardButton','发送')
        self.gbjb=win32gui.FindWindowEx(self.juBingChuangKou.wei_childrenHandle(guodu)[1], None, 'StandardButton','关闭')
        self.liaoTianChuangKou=self.juBingChuangKou.wei_childrenHandle(guodu)[0]

    def duqupeiz(self):#读取配置
        self.sffswcgb=eval(weiabc123.weiConfig(r'.\cof\peizhi').wei_read('config','是否发送完成关闭'))
        self.jzcffs=eval(weiabc123.weiConfig(r'.\cof\peizhi').wei_read('config','禁止重复发送'))
        self.huinr01=weiabc123.weiConfig(r'.\cof\peizhi').wei_read('config','发送内容')
        self.fssd01=float(weiabc123.weiConfig(r'.\cof\peizhi').wei_read('config','发送速度'))
        self.qianniuzhanghao=weiabc123.weiConfig(r'.\cof\peizhi').wei_read('config','发送帐号')

    def fasong(self):
        '''发送一条'''
        self.duqupeiz()#读取配置，保证配置内容为最新
        self.juBingChuangKou.wei_current(self.hwnd)
        for i in range(10):
            self.juBingChuangKou.click_Handle(self.wwsskjb,clicks=2)
        ...
        if dksb:
            ...
            weiabc123.weiLog('发送失败日志.txt').wei_write(shibai)
            print(shibai)
            return
        ...
        if quxiao1:
            ...
            weiabc123.weiLog('发送日志.txt').wei_write(shibai)
            return
        ...
        pyperclip.copy('')
        self.juBingChuangKou.click_Handle(jb=self.liaoTianChuangKou,wz=1,py_x=1,py_y=5)
    ...
    def Control(self):#操控方法
        ...
        if benggongsi:
            if time.time()>1596938898:
                win32api.MessageBox(0, '应用已更新，或者配置错误，详情联系开发者QQ：1043014552',u'提示框',win32con.MB_SYSTEMMODAL)
                return
        else:
            yanZheng009=weiabc123.yanZheng(self.qianniuzhanghao).yanZheng()
            if not yanZheng009 : return
        ...
        try:self.biaoge=xlrd.open_workbook(r'旺旺名.xls').sheets()[0]#接收消息的旺旺表
            ...
            return
        hwnd0=win32gui.FindWindow("StandardFrame",titlename)
        self.hwnd=weiabc123.weiHandleWindow().wei_childrenHandle(hwnd0)[0]#发送旺旺窗口句柄
        ...
        del self.wwid[0]
        readrizhi=weiabc123.weiBigFileRead().wbfr_read('发送日志.txt',500)
        if readrizhi:
            fsdww=re.findall('\n(.*?)\t\t\t',readrizhi)[-1]#发送到的旺旺
        ...
        try:self.find_jb()#取需要的句柄
            ...
            weiabc123.weiLog(r'.\cof\error_log').wei_error()
        ...
        win32gui.PostMessage(win32gui.FindWindow("TkTopLevel",'千牛批量自动发消息'),win32con.WM_CLOSE,0,0)
